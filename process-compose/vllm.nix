...
let
  ...
  baseDockerArgs = [
    ...
    "-p"
    "${toString config.port}:8000"
    "-v"
    "${config.dataDir}:/root/.cache/huggingface"
    "--ipc=host"
  ];
  ...
  metalArgs = [
    "-v"
    "/tmp:/tmp" # Metal needs tmp directory
  ];

  # Additional environment variables per platform
  amdEnvVars = [
    "-e"
    "HSA_OVERRIDE_GFX_VERSION=11.0.0" # May need adjustment per GPU
  ];
